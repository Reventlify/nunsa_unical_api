  ...
  try {
    ...
    WITH PostStats AS (
      ...
      FROM
        ...
        WHERE (sch_session = $2 OR sch_session = $3 OR sch_session = $4 AND sch_session IS NOT NULL) OR (sch_session IS NULL AND $2 IS NULL)
    ...
    const data =
      level === "200"
        ? await pool.query(query, [student_id, helper, "21/22", "20/21"])
        : await pool.query(query, [student_id, helper]);
